...
from modules.utils import extract_stations, sample_station, sample_distinct_station, generate_cars_per_station, assign_train_capacities
...
def check_params(par):
    ...
    required_params = {
        ...
        "min_duration": int,
        "max_duration":  int,
        # the following values are added in the context of this project:

        # a list of train types denoted by integers representing max weight capacity of the train
        "train_types": list, # List[int]
        # an upper bound on the number of cars at a station
        "max_cars_at_stations": int,

        "car_value_range": tuple, # Tuple[int, int]
        "car_weight_range": tuple # Tuple[int, int]
    }
    ...
    if check_params(params):
        ...
        for i in range(start_index, start_index + args.num_envs):
            ...
            env = RailEnv(
                        ...
                        remove_agents_at_target= params.remove_agents_at_target # should always be false in the context of our project?
                        )
            
            # add additional project related parameters
            env.train_types = params.train_types
            env.max_cars_at_stations = params.max_cars_at_stations
            env.car_value_range = params.car_value_range
            env.car_weight_range = params.car_weight_range

            env.reset(random_seed=params.seed) # use random seed here to avoid unwanted randomization side effects

            env.stations = extract_stations(env)

            # function stores car infos
            generate_cars_per_station(env)
            assign_train_capacities(env)
